...
8. Distributed mode: Query the data to find all ngrams in 1993.
    ...
    Drill performs partition pruning when you query partitioned data, which improves performance. Performance can be improved further by casting the yr and occurrances columns to INTEGER, as described in section ["Tips for Performant Querying"]({{site.baseurl}}/docs/text-files-csv-tsv-psv/#tips-for-performant-querying).
